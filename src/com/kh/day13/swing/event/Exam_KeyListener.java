...
public class Exam_KeyListener extends JFrame{
	...
	public Exam_KeyListener() {
		...
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);	// 닫을때까지 닫은게 아님
		...
		for(JLabel la : laArrs) {
			la.setOpaque(true);					// 바탕색 보이게 하기 위해서 컴포넌트를 불투명하게 하기
			la.setBackground(Color.YELLOW);		// 바탕색 노란색으로 설정
			c.add(la);							// 컴포넌트 컨테이너에 추가
		}
//		la1.setOpaque(true); 	// 바탕색 보이게 하기 위해서 컴포넌트를 불투명하게 하기
//		la2.setOpaque(true); 	// 바탕색 보이게 하기 위해서 컴포넌트를 불투명하게 하기
//		la3.setOpaque(true); 	// 바탕색 보이게 하기 위해서 컴포넌트를 불투명하게 하기
