			...
			fruits[i].addItemListener(new ItemListener() {		// 아이템리스터 핸들러 등록
				...
				public void itemStateChanged(ItemEvent e) {		// 아이템 선택될 때
					if(e.getStateChange() == ItemEvent.SELECTED) {	// 선택이 됐으면
						...
						sum += 500;									// 그러면 500을 더하라
						} else if(e.getItem().equals(fruits[2])) {	// 그 이름이 체리이면
							sum += 2000;							// 2000을 더해라
						} 
					} else {										// 체크가 해제되면
						if(e.getItem().equals(fruits[0])) {			// 사과는 100을 빼고
							sum -= 100;
						} else if(e.getItem().equals(fruits[1])) {	// 배는 500을 빼고
						sum -= 500;
						} else if(e.getItem().equals(fruits[2])) {	// 체리는 2000을 빼라
							sum -= 2000;
						}
					}
